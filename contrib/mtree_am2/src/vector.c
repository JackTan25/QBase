...
void PrintLeafPageVectors(char *msg, Page page)
{
	...
	{
		float8 distance = ((MtreeElementLeafTuple)PageGetItem(page, PageGetItemId(page, i)))->distance_to_parent;
		ItemPointerData data_tid = ((MtreeElementLeafTuple)PageGetItem(page, PageGetItemId(page, i)))->data_tid;
		char *out = DatumGetPointer(DirectFunctionCall1(vector_out, PointerGetDatum(&((MtreeElementLeafTuple)PageGetItem(page, PageGetItemId(page, i)))->vec)));
...
void PrintInternalPageVectors(char *msg, Page page)
{
	...
	for (int i = FirstOffsetNumber; i <= offsets; i = OffsetNumberNext(i))
	{
		// Vector vec = ((MtreeElementTuple)PageGetItem(page, PageGetItemId(page, i)))->vec;
		float8 distance = ((MtreeElementTuple)PageGetItem(page, PageGetItemId(page, i)))->distance_to_parent;
		float8 radius = ((MtreeElementTuple)PageGetItem(page, PageGetItemId(page, i)))->radius;
		BlockNumber son_page = ((MtreeElementTuple)PageGetItem(page, PageGetItemId(page, i)))->son_page;
		char *out = DatumGetPointer(DirectFunctionCall1(vector_out, PointerGetDatum(&((MtreeElementTuple)PageGetItem(page, PageGetItemId(page, i)))->vec)));
