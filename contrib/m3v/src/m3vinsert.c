...
void WriteElementToPage(m3vElement e, Page page,int columns)
{
	/* Calculate sizes */
	Size etupSize = 0;
...
void WriteLeafElementToPage(m3vElement e, Page page,int columns)
{
	/* Calculate sizes */
	Size etupLeafSize = 0;
...
bool m3vInsertTuple(Relation index, m3vElement element, bool *isnull, ItemPointer heap_tid, Relation heapRel, m3vBuildState *buildstate, GenericXLogState *state,int columns)
{
	...
	// PrintVector("insert vector ", element->vec);
	// strat record log.
	Insertm3v(metap->root, index, element, isnull, heapRel, procinfo, collation, state,columns);
	// DebugEntirem3vTree(5, index, 0);
	// DebugEntirem3vTree(3, index, 0);
	elog(INFO, "new root a: %d", m3vGetMetaPageInfo(index).root);
	DebugEntirem3vTree(m3vGetMetaPageInfo(index).root, index, 0,columns);
	return true;
}
...
bool Insertm3v(BlockNumber root_block, Relation index, m3vElement element, bool *isnull, Relation heapRel, FmgrInfo *procinfo, Oid collation, GenericXLogState *state,int columns)
{
	...
	M3V_ELEMENT_LEAF_TUPLE_SIZES(element->vecs,columns,etupLeafSize);
	// etupLeafCombineSize = etupLeafSize + sizeof(ItemIdData);
	etupLeafCombineSize = etupLeafSize;
	/**
	...
	if (PageType(m3vPageGetOpaque(page)->type) == M3V_LEAF_PAGE_TYPE)
	{
		...
		{
			// push directly
			WriteLeafElementToPage(element, page,columns);
		...
		else
		{
			...
			if (is_root(m3vPageGetOpaque(page)->type))
			{
				 ...
				 */
				PrintLeafPageVectors("left page: ", page,columns);
				PrintLeafPageVectors("right page: ", new_page,columns);
				PrintInternalPageVectors("root page: ", new_root_page,columns);
			...
			else
			{
				...
				UpdateParentRecurse(parent_page, parent_blkno, index, procinfo, collation, temp_page, new_page, left_etup, right_etup, m3vPageGetOpaque(temp_page)->offset, state,columns);
				// restore
				PageRestoreTempPage(temp_page, page);
				PrintLeafPageVectors("test leaf page", temp_page,columns);
				PrintLeafPageVectors("test new leaf page", new_page,columns);
				PrintInternalPageVectors("after UpdateParentRecurse ", parent_page,columns);
				...
				DebugEntirem3vTree(3, index, 0,columns);
			}
		}
	}
	else
	{
		...
		append_only = Insertm3v(son_page, index, element, isnull, heapRel, procinfo, collation, state,columns);
	}
	if (append_only)
	{
		UnlockReleaseBuffer(buf);
...
void UpdateParentRecurse(Page parent_page, BlockNumber parent_block_num, Relation index, FmgrInfo *procinfo, Oid collation, Page left_son_page, Page right_son_page, m3vElementTuple left_centor, m3vElementTuple right_centor, OffsetNumber left_offset, GenericXLogState *state,int columns)
{
	...
	for(int i = 0;i < columns;i++){
		etupSize += M3V_ELEMENT_TUPLE_SIZE(left_centor->vecs[i].dim);
	}
	...
	if (PageGetMaxOffsetNumber(parent_page) >= 2)
	{
		...
		Page temp_page = SplitInternalPage(parent_page, new_page, procinfo, collation, left_centor, right_centor, left_offset, &left_copy_up, &right_copy_up, &left_radius, &right_radius,columns);
		...
		if (is_root(m3vPageGetOpaque(parent_page)->type))
		{
			...
			PrintInternalPageVectors("test parent_page", parent_page,columns);
		...
		else
		{
			...
			UpdateParentRecurse(new_parent_page, m3vPageGetOpaque(parent_page)->parent_blkno, index, procinfo, collation, temp_page, new_page, DatumGetm3vElementTuple(left_copy_up), DatumGetm3vElementTuple(right_copy_up), m3vPageGetOpaque(parent_page)->offset, state,columns);
			// restore temp page
			...
			UnlockReleaseBuffer(right_buf);
			DebugEntirem3vTree(parent_block_num, index, 0,columns);
	...
	else
	{
		...
		PrintInternalPageVectors("after replace item", parent_page,columns);
