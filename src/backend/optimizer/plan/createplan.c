					  ...
					  bool indexonly)
{
	...
	if (indexorderbys)
	{
		 ...
		 */
		// Hack, if it'a multi-column vector search, the indexorderbys's length will be
		// larger than the pathkeys'.
		if (best_path->is_vector_search)
		{
			if (list_length(best_path->path.pathkeys) != 1)
			{
				elog(ERROR, "for vector search, we can have only one PathKey");
			}
			PathKey *pathkey = (PathKey *)(list_head(best_path->path.pathkeys));
			foreach (exprCell, indexorderbys)
			{
